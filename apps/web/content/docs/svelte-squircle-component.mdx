...
- **Fixed dimensions known at authoring time** → [`StaticSquircle`](/docs/svelte-static-squircle). Cheaper (no observer).
...
- [Examples & Recipes](/docs/svelte-examples) — `Squircle` in real button, card, and avatar patterns.
- [StaticSquircle](/docs/svelte-static-squircle) — the cheaper, SSR-friendly option for fixed-size elements.
